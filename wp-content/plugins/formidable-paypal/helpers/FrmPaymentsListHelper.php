...
class FrmPaymentsListHelper extends WP_List_Table {
    ...
    function prepare_items() {
    	...
    	$this->items = $wpdb->get_results("SELECT * FROM {$wpdb->prefix}frm_payments ORDER BY $orderby $order LIMIT $start, $per_page");
    	$total_items = $wpdb->get_var("SELECT COUNT(*) FROM {$wpdb->prefix}frm_payments");
...
<?php
    		...
    		foreach ( $columns as $column_name => $column_display_name ) {
    			...
    			switch ( $column_name ) {
    				...
    				case 'receipt_id':
    					$val = '<strong><a class="row-title" href="' . esc_url( $edit_link ) . ' title="' . __( 'Edit' ) . '">' . $item->receipt_id . '</a></strong><br />';
    				...
    				case 'user_id':
                        ...
                        break;
    				case 'item_id':
						$val = '<a href="' . '?page=formidable-entries&frm_action=show&action=show&id='.$item->item_id . '">' . $item->item_id . '</a>';
    					break;
    				case 'form_id':
    				...
    				case 'expire_date':
						...
						if( $datetime < 1 ) {
							...
							$val = '<abbr title="' . date( $date_format . ' g:i:s A', $datetime ) . '">' . $date . '</abbr>';
    				    }
