...
class FrmZapApiController{
    ...
    public static function api_route() {
        ...
        if ( !is_user_logged_in() || !current_user_can('administrator') ){
        	...
        	$user = get_userdata(reset($admins->results));
        } else {
    ...
    private static function forms($data) {
	    ...
	    $forms = array('forms' => (array) $frm_form->getAll( array('is_template' => 0, 'status' => 'published') ));
    ...
    private static function fields($data, $user, $atts) {
	    ...
	    $fields = array('fields' => (array) $frm_field->getAll($where, 'field_order'));
	    return $fields;
    }
